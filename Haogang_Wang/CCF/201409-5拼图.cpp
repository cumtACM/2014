...
void update(int a,int b){
	if(b&(1<<(m+1))){
		dp[cur][b^(1<<(m+1))]=(dp[cur][b^(1<<(m+1))]+dp[cur^1][a])%mod;
	} 
}
int main(void){
	while(scanf("%lld%d",&n,&m)==2){
		memset(dp,0,sizeof(dp));
		cur=0;dp[0][(1<<(m+1))-1]=1;
		for(int i=0;i<n;i++){
			for(int j=0;j<m;j++){
				cur^=1;
				...
				for(int k=0;k<(1<<(m+1));k++){
					update(k,k<<1);
					if(i&&j&&!(k&(1<<(m-1)))&&!(k&(1<<m)))
						update(k,(k<<1)^(1<<m)^(1<<(m+1))^1);
					if(i&&j<m-1&&!(k&(1<<(m-1)))&&!(k&(1<<(m-2))))
						update(k,(k<<1)^(1<<m)^(1<<(m-1))^1);
					if(i&&j&&!(k&(1<<(m-1)))&&!(k&1))
						update(k,(k<<1)^(1<<m)^3);
					if(i&&j&&!(k&(1<<m))&&!(k&1))
						update(k,(k<<1)^3^(1<<(m+1))); 
				}
			}
		}
		printf("%lld\n",dp[cur][(1<<(m+1))-1]);
